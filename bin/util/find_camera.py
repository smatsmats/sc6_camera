...
for n in range(fourth_start, fourth_stop):
    ...
    if print_dots:
        ...
        if (n-1) % 50 == 0 and n > 1 and n < 249:
            print()
#    print(ip)
    command = "curl -s -w '%{{http_code}}\n' -o /dev/null --request GET --url 'http://{}/jpeg.cgi'".format(ip)
    ...
    if print_dots and s_code != '000':
        print()
    if s_code == '200':
        print('{} is a {} ({})!!!!!!!!!!'.format(ip, thing, s_code))
    elif s_code == '401':
        print('{} might be a {} you\'re not authorized for ({})'.format(ip, thing, s_code))
    elif s_code == '404':
        print('{} is something other than a {} ({})'.format(ip, thing, s_code))
