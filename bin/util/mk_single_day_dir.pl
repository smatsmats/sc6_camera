...
sub wanted {
    ...
    my $link_dest = readlink($current);

    my $file_part = basename($link_dest); 
    my $new_link = $single_dir . $file_part;
    print "link $link_dest to $new_link\n";
    symlink($link_dest, $new_link) or die "Can't symlink $link_dest to $new_link: $!\n";
}
